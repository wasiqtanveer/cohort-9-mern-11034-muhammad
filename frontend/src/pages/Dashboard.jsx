...
function Dashboard(props)
{
  ...
  const visibleNotes = useMemo(() => {
    ...
    //copy first, .sort() mutates in place and this array is App's state
    return [...filtered].sort((a, b) => {
      //pinned stay on top whichever sort is picked
  ...
  async function handlePin(note)
  {
    setError('');
    try {
      //send the value we want, not a toggle, so a double click cant desync
    ...
    <AppShell>
          ...
          {notes.length > 0 && (
            <div className='mb-10 flex flex-col gap-3 sm:flex-row sm:items-center'>
              {/* field and picker share one pill so it reads as "searching by this" */}
              ...
              {/* no fixed width so the arrow sits right after the text */}
              <select
                value={sortBy}
          ...
          {/* placeholders while the list is in flight, otherwise "No notes yet" flashes on every load */}
          {notesLoading && (
          ...
          <div className='grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4'>
            {visibleNotes.map((note)=>(
              <div key={note.id} className={`group relative flex flex-col rounded-2xl p-5 transition hover:-translate-y-0.5 hover:shadow-lg ${getCardColor(note.id)}`}>
                ...
                {/* stays visible while pinned, otherwise shows on hover or keyboard focus */}
                <button
                  type='button'
...
/*props expected by the Dashboard component*/
Dashboard.propTypes = {
